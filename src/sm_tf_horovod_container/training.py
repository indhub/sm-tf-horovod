...
from sm_tf_horovod_container.timeout import timeout
...
def train(env, hyperparameters):
    """Runs training on a user supplied module in either a local or distributed
    ...
    * `sagemaker_use_mpi`: force use of MPI so that scripts can be run on a single host.
    ...
    * `sagemaker_additional_mpi_options`: a string of options to pass to mpirun.
    ...
    # Move data to memory if requested
    # Change the environment variable
    # Change command line param that gets passed to mpirun
    if bool(hyperparameters.get('sagemaker_use_tmpfs')):
        ram_disk_size = hyperparameters.get('sagemaker_tmpfs_size')
        if ram_disk_size is None:
            logger.error("If sagemaker_use_tmpfs is specified, sagemaker_tmpfs_size must be specified")
            return False

        alt_training_dir = "/mnt/ramdisk/training"
        content = Template(textwrap.dedent("""\
            #!/usr/bin/env bash
            mkdir /mnt/ramdisk
            mount -t tmpfs -o size=$size tmpfs $dst
            cp -r $src $dst
            EXIT_CODE=$$?
            exit $${EXIT_CODE}
            """))
        content = content.substitute(size = "%dg" % ram_disk_size,
                src = env.channel_input_dirs['training'],
                dst = alt_training_dir)

        CREATE_TMPFS_SCRIPT = "/create_tmp_fs.sh"
        with open(CREATE_TMPFS_SCRIPT, 'w') as w:
            w.write(content)

        subprocess.check_call(["bash", CREATE_TMPFS_SCRIPT])
        env.channel_input_dirs['training'] = "/mnt/ramdisk/training"
...
@retry(stop_max_delay=10 * 60 * 1000, wait_fixed=1000, retry_on_result=_retry_if_false)
